...
- [ ] **T-CUT04** — Go: `game.agp` `[cutscenes]` and `[input]` blocks — `[cutscenes]`: `fallback_debug`, `fallback_release`, `fallback_qa`, `step_timeout_default`. `[input]`: `dialogue_advance`, `cutscene_skip`, `dialogue_hold_advance` input action bindings. Parse into the existing `Manifest` struct in `tools/ag/internal/project/`. Tests for each field. *(no dependencies)*
