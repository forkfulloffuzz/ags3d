...
func test_01_step_completes_before_timeout() -> void:
	...
	seq.step_complete.connect(func(_id: String) -> void: completed = true)
	await seq.run([{"type": "wait", "duration": 0.0, "timeout": 1.0, "id": "fast"}])
	assert_true(completed, "step that completes before timeout should fire step_complete")
	await _cleanup(seq)
...
func test_02_timeout_none_no_limit() -> void:
	...
	assert_true(completed, "step with timeout:none should complete normally")
	await _cleanup(seq)
...
func test_03_global_default_applied() -> void:
	...
	assert_true(completed, "global default timeout should allow fast step to complete")
	await _cleanup(seq)
...
func test_04_perstep_timeout_overrides_global() -> void:
	...
	assert_true(completed, "per-step timeout should override short global default")
	await _cleanup(seq)
